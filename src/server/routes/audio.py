...
async def transcribe(request):
    """纯转写端点: 上传音频 -> Whisper/FunASR -> 返回文本。

    不调 LLM, 不驱动数字人。供前端 (Edge / 网络受限环境) 替代浏览器 Web Speech API。
    """
    try:
        ...
        if fileobj is None:
            return web.Response(
                ...
                text=json.dumps({'code': -1, 'msg': '缺少 file 字段'}, ensure_ascii=False),
            )
        filebytes = fileobj.file.read()
        size = len(filebytes)
        import time
        t0 = time.perf_counter()
        logger.info(f'[transcribe] received {size} bytes, type={fileobj.content_type}')
        ...
        content_type = getattr(fileobj, 'content_type', None) or form.get('content_type')
        result = await loop.run_in_executor(
            None,
            lambda: asr_engine.transcribe(filebytes, content_type=content_type),
        )
        text = (result or {}).get('text', '').strip()
        elapsed_ms = int((time.perf_counter() - t0) * 1000)
        logger.info(f'[transcribe] -> {text!r} ({elapsed_ms}ms)')

        return web.Response(
            content_type='application/json',
            text=json.dumps({'code': 0, 'msg': 'ok', 'data': {'text': text}}, ensure_ascii=False),
        )
        ...
        return web.Response(
            status=500,
            content_type='application/json',
            text=json.dumps({'code': -1, 'msg': str(e)}, ensure_ascii=False),
        )
