...
class Wav2LipAvatar(BaseAvatar):
    ...
    def render(self,quit_event,loop=None,audio_track=None,video_track=None):
        ...
        while not quit_event.is_set(): 
            ...
            # 口型帧率 = audio_fps/2; 队列积压时同时看音视频，避免只憋视频导致声画不同步
            video_ptime = 2.0 / self.config.audio.fps
            vq = video_track._queue.qsize() if video_track else 0
            aq = audio_track._queue.qsize() if audio_track else 0
            backlog = max(vq, aq // 2)
            if backlog >= 3:
                logger.debug('sync throttle vq=%d aq=%d', vq, aq)
                time.sleep(video_ptime * backlog * 0.75)
