...
AUDIO_PTIME = 0.020  # 默认 50fps 音频分片 (20ms)
VIDEO_CLOCK_RATE = 90000
VIDEO_TIME_BASE = fractions.Fraction(1, VIDEO_CLOCK_RATE)
SAMPLE_RATE = 16000
AUDIO_TIME_BASE = fractions.Fraction(1, SAMPLE_RATE)
# Wav2Lip: 每帧视频对应 2 个音频 chunk, 视频间隔 = 2 * 音频间隔
AUDIO_FRAMES_PER_VIDEO = 2
...
class PlayerStreamTrack(MediaStreamTrack):
    ...
    def _media_start(self) -> float:
        """音视频共用同一时间轴，避免口型与声音各走各的时钟。"""
        player = self._player
        if player._media_start is None:
            player._media_start = time.time()
            mylogger.info('media clock start: %f (audio_fps=%s)', player._media_start, player.audio_fps)
        return player._media_start

    def _ptime(self) -> float:
        player = self._player
        if self.kind == 'video':
            return player.video_ptime
        return player.audio_ptime
    ...
    async def next_timestamp(self) -> Tuple[int, fractions.Fraction]:
        ...
        ptime = self._ptime()
        self._start = self._media_start()

        if self.kind == 'video':
            time_base = VIDEO_TIME_BASE
            step = int(ptime * VIDEO_CLOCK_RATE)
        else:
            time_base = AUDIO_TIME_BASE
            step = int(ptime * SAMPLE_RATE)
...
class HumanPlayer:
    ...
    def __init__(
        ...
        # 与 Wav2Lip 产帧一致: 50fps 音频 + 25fps 口型 (每视频帧 2 个音频 chunk)
        cfg = getattr(avatar_stream, 'config', None)
        self.audio_fps = cfg.audio.fps if cfg else 50
        self.audio_ptime = 1.0 / self.audio_fps
        self.video_ptime = self.audio_ptime * AUDIO_FRAMES_PER_VIDEO
